...
(function () {
  ...
  var API_BASE = '';
  var AUTH_SMS_REQUEST_PATH = '/auth/sms/request';
  var AUTH_SMS_VERIFY_PATH = '/auth/sms/verify';
  ...
  function applyAccountState(data) {
    ...
    if (data.authenticated === true || data.phone_verified === true || account.phone_mask || account.phone_masked) {
      state.phoneVerified = true;
      state.phoneMask = String(account.phone_mask || account.phone_masked || data.phone_mask || data.phone_masked || '');
    }
    return data;
  }
  ...
  window.zxMember = {
    ...
    phoneVerified: function () { return state.phoneVerified; },
    me: refresh,
    authSession: authSession,
    requestSms: function (phone, risk) {
      risk = risk && typeof risk === 'object' && !Array.isArray(risk) ? risk : {};
      return api(AUTH_SMS_REQUEST_PATH, {
        method: 'POST',
        body: {
          phone: String(phone || '').trim(),
          purpose: 'login',
          risk: {
            ticket: String(risk.ticket || '').trim(),
            randstr: String(risk.randstr || '').trim()
          }
        }
      });
    },
    verifySms: function (challengeId, phone, code) {
      return api(AUTH_SMS_VERIFY_PATH, {
        method: 'POST',
        body: {
        ...
        state.phoneMask = String(account.phone_mask || account.phone_masked || '');
        return refresh().catch(function () { state.ready = true; return data; });
      });
    },
    logout: function () {
      function sendLogout() {
    ...
    membershipProducts: function () { return api('/membership/products'); },
    createPaymentOrder: function (input, idempotencyKey) {
      input = input || {};
      return api('/payments/orders', {
        ...
        body: {
          ...
          channel: 'wechatpay_h5',
          consent: input.consent === true,
          agreement_version: String(input.agreementVersion || ''),
          privacy_version: String(input.privacyVersion || ''),
          membership_terms_version: String(input.membershipTermsVersion || ''),
          refund_policy_version: String(input.refundPolicyVersion || ''),
          ai_disclosure_version: String(input.aiDisclosureVersion || ''),
          purchase_notice_version: String(input.purchaseNoticeVersion || ''),
          adult_confirmed: input.adultConfirmed === true
        }
      });
    },
